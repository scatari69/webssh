...
**После обновления фронтенда сделайте жёсткое обновление страницы**
(Ctrl+Shift+R, на мобильном — перезагрузка вкладки). Страницы отдаются с
`no-store` и приходят новыми сразу, а `/js` и `/css` кешируются на час:
браузер может подставить свежий HTML к вчерашнему JavaScript. Имена файлов
не содержат хеша версии, поэтому автоматически это не разрешается — если
интерфейс после обновления ведёт себя странно, дело почти наверняка в этом.

Остановка:

```bash
docker compose down       # остановить, данные сохранить
docker compose down -v    # остановить и стереть тома вместе с данными
```
