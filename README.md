...
мобильная раскладка со спецклавишами. Панель администрирования пока
сводится к сводке — управление через REST API, см. «Что дальше».
...
1. **Панель администрирования.** `/admin` сейчас показывает только сводку;
   управление пользователями и SSH-конфигурацией — через REST API.
...
3. **Переподключение после обрыва.** Сейчас закрытие WebSocket убивает
...
4. Просмотр журнала аудита и списка живых сессий в админке, ротация по
   `AUDIT_RETENTION_DAYS`.
5. CSRF-токен для мутирующих запросов. Сейчас защита опирается на
