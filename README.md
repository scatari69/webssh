...
**Текущее состояние.** Готовы конфигурация, схема БД, шифрование секретов,
аутентификация с ролями и двухфакторкой и админский REST API.
SSH-подключений, терминала и фронтенда ещё нет — см. «Что дальше».
...
Два теста на зашифрованные SSH-ключи требуют `ssh-keygen` (собрать такой
ключ без него нельзя — нужен bcrypt_pbkdf). Если его нет, они помечаются
`SKIP` с указанием причины; в alpine-образе доустанавливается через
`apk add openssh-keygen`. Остальные ключи тесты генерируют сами, чтобы не
держать приватный ключ фикстурой в репозитории.
...
1. SSH и WebSocket-терминал: проверка ключа хоста (TOFU), лимиты сессий,
   переподключение с проигрыванием буфера после обрыва.
2. Фронтенд: xterm.js, темы Catppuccin, контекстное меню, мобильный ввод.
   QR-код для привязки TOTP рисуется там же из `otpauth_uri`.
3. Просмотр журнала аудита в админке и его ротация по
