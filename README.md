...
4. CSRF-токен для мутирующих запросов. Сейчас защита опирается на
   `sameSite=lax` (браузер не отправляет cookie при межсайтовом POST) и на
   требование `Content-Type: application/json`, который межсайтовая форма
   выставить не может. Для текущего API этого достаточно, но явный токен
   надёжнее.
