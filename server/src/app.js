...
function createApp() {
   ...
   *
   * Кеш — `no-cache`, то есть «бери из кеша, но сперва переспроси». Вместе
   * с ETag это даёт 304 без тела, пока файл не менялся, и свежий файл сразу
   * после обновления.
   *
   * Раньше здесь стоял max-age=1h, и это давало неочевидный и неприятный
   * эффект: страницы отдаются с no-store и после выкатки приходят новыми, а
   * модули к ним — старыми, из кеша. Свежая разметка встречалась с
   * вчерашним кодом, и всё это выглядело как «обновление не применилось».
   * Имён с хешем содержимого у файлов нет (сборщика нет намеренно), а
   * версионировать URL вручную нельзя: модули импортируют друг друга по
   * относительным путям, и запрос в адресе до них не доходит. Поэтому
   * правильный размен здесь — условный запрос на файл вместо гадания о
   * сроке годности.
   */
  const staticOptions = {
    index: false,
